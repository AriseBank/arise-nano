...
export const extractPublicKey = passphrase =>
  Arise.crypto.getKeys(passphrase).publicKey;
...
export const extractAddress = (data) => {
  if (data.indexOf(' ') < 0) {
    return Arise.crypto.getAddress(data);
  }
  const { publicKey } = Arise.crypto.getKeys(data);
  return Arise.crypto.getAddress(publicKey);
};
